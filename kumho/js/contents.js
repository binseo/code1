$(document).ready(function(){
    ...
    let obj_photo // = $('.ctn_history .his_photo')
    let obj_photo_top
    let obj_photo_show
    ...
    // offset().top --> 최상의노드인 문서(document) 위치를 시작점?
    // eq --> 몇번째꺼의 값

    // console.log(obj_photo.length) // length --> photo 몇개?

    function scroll_chk(){
        scrolling = $(window).scrollTop()
        // console.log('스크롤 값', scrolling, 'photo', obj_photo_top, '브라우저', window_h)
    }
    function resize_chk(){
    ...
    function photo_show(){
        for(i = 0; i < obj_name.length; i++){ // i++ --> 1씩 증가
            // console.log(i)
    ...
    function nav_show(){
        // obj_nav = $('.ctn_history .his_nav')
        // obj_nav_area = $('.ctn_history')
        // obj_nav_start // 보이기 시작하는 스크롤 값
        // obj_nav_end // 보이는 마지막 스크롤 값
